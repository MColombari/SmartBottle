...
#define GY_G_FORCE_SLACK 500
#define GY_ZERO_SLACK 1000
...
void loop() {
  ...
  if(GY_UPDATE_FLAG_PERIOD <= millis() - start_time_update){
    ...
    if(
      ((mean_X >= -GY_ZERO_SLACK) && (mean_X <= GY_ZERO_SLACK)) &&
      ((mean_Y >= -GY_ZERO_SLACK) && (mean_Y <= GY_ZERO_SLACK)) &&
      ((mean_Z >= GY_G_FORCE_VALUE - GY_G_FORCE_SLACK) && (mean_Z <= GY_G_FORCE_VALUE + GY_G_FORCE_SLACK))){
        // Print if changed.
        if(is_vertical == false){
          Bluetooth.println("[GY-521]: Is vertical");
        }
        is_vertical = true;
    }
    else{
      if(is_vertical == true){
        Bluetooth.println("[GY-521]: Is NOT vertical");
      }
      is_vertical = false;
    }
